...
- **Linter**: Biome
- **ORM**: Drizzle (PostgreSQL via Postgres.JS)
...
bun test         # Run tests
bun generate     # Generate Drizzle migrations (after schema change)
bun push         # Push schema to DB without migration (dev only)
...
> Never edit migration files manually — always use `bun x drizzle-kit generate`.
...
| Schema changed | Database section |
| New language added | I18n section |