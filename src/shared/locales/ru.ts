...
export const ru = {
	...
	// Manual order payment proof (crypto / zarinpal → admin review)
	orderCryptoProofInstructions:
		`${e.clipboard} <b>Шаги оплаты:</b>\n\n` +
		`1. Отправьте точную сумму на адрес выше\n` +
		`2. Затем пришлите сюда хэш транзакции (TxID) или фото чека\n` +
		`3. Заказ будет обработан после проверки и подтверждения администратором`,
	orderZarinpalProofInstructions:
		`${e.clipboard} <b>Шаги оплаты:</b>\n\n` +
		`1. Нажмите «Оплатить» и завершите оплату\n` +
		`2. Затем пришлите сюда код отслеживания или фото чека\n` +
		`3. Заказ будет обработан после проверки и подтверждения администратором`,
	orderProofExpect: `${e.reject} Пожалуйста, отправьте хэш транзакции (текст) или фото чека об оплате.`,
	...
	ordersOrderLabel: (data: { orderId: number }) => `Заказ #${data.orderId}`,
	ordersProductFallback: `Товар`,
	ordersDeliveredLabel: `Доставка`,
	...
	orderDeliveryInfoNotProvided: "Информация о доставке не указана.",

	// Order Statuses
	orderStatus_pending_payment: "Ожидает подтверждения оплаты",
	orderStatus_paid: "Оплачено",
	orderStatus_pending_admin: "Ожидает проверки админом",
	orderStatus_waiting_schedule: "Требуется выбор времени",
	orderStatus_scheduled: "Запланировано",
	orderStatus_reminder_sent: "Напоминание отправлено",
	orderStatus_waiting_user_online: "Ожидание вашего присутствия",
	orderStatus_user_not_responding: "Нет ответа",
	orderStatus_waiting_invite: "Ожидание отправки приглашения",
	orderStatus_invite_sent: "Приглашение отправлено",
	orderStatus_waiting_user_action: "Требуется ваше действие",
	orderStatus_join_link_sent: "Ссылка Join отправлена",
	orderStatus_in_queue: "В очереди",
	orderStatus_in_progress: "В процессе",
	orderStatus_active: "Активен",
	orderStatus_expiring_soon: "Скоро истекает",
	orderStatus_completed: "Завершён",
	orderStatus_cancelled: "Отменён",
	orderStatus_refunded: "Возврат средств",
	orderStatus_failed: "Неуспешно",
	orderStatus_rescheduled: "Время изменено",
	...
	paymentQuantity: "Количество",
	enterOrderQuantityPrompt: `${e.trolley} Сколько единиц этого товара вы хотите?\n\nВведите количество числом:`,
