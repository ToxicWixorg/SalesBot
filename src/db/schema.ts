...
export const zarinpalWalletPaymentsTable = pgTable(
	"zarinpal_wallet_payments",
	{
		...
		status: text("status").notNull().default("pending"), // pending | verified | credited | failed | cancelled
		refId: text("ref_id"),
...
export const nowpaymentsWalletPaymentsTable = pgTable(
	"nowpayments_wallet_payments",
	{
		...
		userId: bigint("user_id", { mode: "number" })
			.notNull()
			.references(() => usersTable.id, { onDelete: "cascade" }),
		amount: decimal("amount", { precision: 15, scale: 2 }).notNull(), // Toman
...
export const botSettingsTable = pgTable("bot_settings", {
	...
	// USD amount credited to the referrer when they invite a new user.
	referralRewardAmount: decimal("referral_reward_amount", {
		precision: 15,
		scale: 2,
	}).default("1.00"),
